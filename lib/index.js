...
/**
 * Simplified regex to match valid identifiers.
 * It may produce false negatives causing some (unusual) property names
 * being printed between quotes.
 */
 ...
 * @param {string} [options.indentSize] - Number of spaces used for an indentation level.
 ...
 * @param {boolean} [options.annotated] - Annotate program models.
 * @param {integer} [options.breakLength=78] - Maximal line width.
 * *Note:* `text()` and `format()` methods don't respect this option.
 */
...
Printer.prototype = {
   ...
   * Flushes the last line and prints a line break if needed.
   *
   * @returns the printer.
   */
  br() {
   ...
   */
  rule(pattern = '-') {
    let len = this.options.breakLength - this.lineLength;
    if (len < 8) { len = 8; }
    return this._emit(pattern.repeat(len)).br();
  },
  ...
  _array(node) {
    ...
    if (node.length === 0) {
      ...
      this._startBlock({ open: '[', close: ']', separator: ',' });
  ...
  _object(node) {
    ...
    if (keys.length === 0) {
      ...
      this._startBlock({ open: '{', close: '}', terminator: ';', spacesAround: true });
      for (let key of keys) {
  ...
  _endBlock() {
    ...
    if (!block.empty && block.spacesAround) {
      this._emit(' ');
    }
  ...
  _startItem() {
    if (this.block.empty) {
      ...
      if (this.block.spacesAround) {
        this._emit(' ');
      }
    } else {
      if (this.block.separator) {
        this._emit({ value: this.block.separator });
      }
      this._emit(' ');
    }
    return this;
  ...
  _endItem() {
    if (this.block.terminator) {
      this._emit({ value: this.block.terminator });
    }
    return this;
  },
  ...
  /**
   * Emits a string or a formatter object to the output.
   * If `value` is an object, it must have the following properties:
   *
   * @param {any} value - A string or a formatter object to emit.
   */
  _emit(value, stylefn) {
    var toBreak = false;
    if (typeof value === "object") {
      ...
      toBreak = this.lineLength + value.length > this.options.breakLength;
